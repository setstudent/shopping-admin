...
- 權限控管與 JWT 驗證：設計管理員角色與權限，整合 JWT 進行身分驗證，保護後台 API 存取。
- 管理員登入流程：負責 Admin 登入頁面與驗證流程，包含錯誤提示與登入後導向後台主畫面。
- 報表匯出與業績視覺化：製作銷售／業績報表，支援匯出（Excel／CSV）與圖表顯示銷售趨勢。
- 業績比對與分析：提供依時間、商品、賣家等條件的業績比對，用於觀察成長與異常。
- 買賣家管理：在後台管理買家與賣家帳號（查詢、檢視、啟用／停用），協助平台帳號與風險控管。

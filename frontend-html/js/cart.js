...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const topupModalOverlay = document.getElementById("topup-modal-overlay"); // (假設這是您 cart.html 中的 ID)
    ...
    const topupForm = document.getElementById("topup-form"); // (抓取表單
    ...
    function checkBalanceForCheckout(totalPrice) {
        ...
        checkoutBtn.textContent = `結帳 (TWD $${totalPriceNum.toFixed(2)})`;
        ...
        if (cartIsEmpty) {
            ...
            checkoutBtn.textContent = `結帳 (餘額不足 TWD $${neededAmount.toFixed(2)})`;
            if (!cartIsEmpty) {
                showMessage('您的錢包餘額不足，請先儲值。', 'warning');
            }
        } else {
    ...
    // --- (J) (核心) 處理結帳 (POST /api/orders/checkout) ---
    ...
    // (綁定「確認儲值」按鈕)
    // (修正) 您的 HTML 中 confirmTopupBtn 的 type="submit"
    // 我們應該監聽 "form" 的 "submit" 事件，而不是按鈕的 "click" 事件
    if (topupForm) {
    ...
    // --- (G) (核心) 處理更新商品數量 (PUT /api/cart/items/{cartItemId}) (已修正錯誤處理) ---
    async function handleUpdateQuantity(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // ***** [已修正錯誤處理區塊：統一讀取純文字，再嘗試解析 JSON] *****
                // 1. 統一讀取回應主體為純文字 (即使是 JSON 也會讀成字串)
                const errorBody = await response.text();
                let errorMessage = '庫存不足或數量無效'; // 預設錯誤訊息

                try {
                    // 2. 嘗試將純文字解析為 JSON
                    const errorData = JSON.parse(errorBody);
                    
                    // 3. 如果解析成功，提取 "message" 欄位
                    if (errorData && errorData.message) {
                        errorMessage = errorData.message;
                    } else {
                        // 如果解析成功但沒有 message 欄位，使用原始字串 (例如整個 JSON)
                        errorMessage = errorBody;
                    }
                } catch (e) {
                    // 4. 如果 JSON 解析失敗 (代表後端回傳的是純文字，例如: "商品庫存不足 (庫存: 20)")
                    if (errorBody) {
                        errorMessage = errorBody;
                    }
                }
            ...
            // [最外層 catch：僅處理網路連線中斷、CORS 錯誤等底層問題]
